-- Simplify daily challenges: just Daily Remix + Daily Gauntlet
-- Gauntlet rotates between card remix (8+ illustrations) and tribe VS
...
DECLARE
  ...
  v_tribe TEXT;
BEGIN
  -- Check if already generated (now only 2 challenges)
  ...
  -- =========================================================================
  -- 2. Daily Gauntlet: alternates card remix (Mon/Wed/Fri) and tribe VS
  -- =========================================================================
  IF v_day_of_week IN (1, 3, 5) THEN
    -- Card remix gauntlet: card with 8+ illustrations, capped at 10
    ...
    -- Build pool from illustrations (capped at 10)
    ...
    v_gauntlet_description := 'King of the hill: ' || v_gauntlet_card_name || ' art';
    ...
    LIMIT 1;
  ELSE
    -- Tribe VS gauntlet: 10 random creatures of a random tribe
    v_gauntlet_mode := 'vs';

    -- Pick a random creature subtype with 10+ cards
    SELECT sub.tribe INTO v_tribe
    FROM (
      SELECT TRIM(UNNEST(STRING_TO_ARRAY(SPLIT_PART(oc.type_line, E'\u2014', 2), ' '))) AS tribe
      FROM oracle_cards oc
    ...
    -- Build pool from 10 random creatures of that tribe
    SELECT jsonb_agg(jsonb_build_object(
      'oracle_id', sub.oracle_id,
      'illustration_id', sub.illustration_id,
      'name', sub.name,
    ...
    FROM (
      ...
      JOIN sets s ON s.set_code = p2.set_code
      WHERE oc.type_line LIKE '%' || v_tribe || '%'
    ...
    v_gauntlet_title := v_tribe || ' Gauntlet';
    v_gauntlet_description := 'King of the hill: 10 random ' || v_tribe || 's';
