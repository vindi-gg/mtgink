-- Surface printings.is_full_art on the three illustration-listing RPCs so
-- the lightbox can display the normal card image (which IS the artwork)
-- rather than the cropped art rectangle for borderless / showcase /
-- extended-art / full-art treatments.
-- Postgres won't change a RETURNS TABLE shape via CREATE OR REPLACE; drop
-- and recreate.
...
AS $func$
  ...
  SELECT
    ...
    elo_rating, vote_count, cheapest_price, is_full_art, scryfall_id, total_count
  FROM combined
  ORDER BY
